...
- **main.py** — Entry point. `WhisprApp` subclasses `rumps.App` for the menu bar UI. Registers a global hotkey listener (`pynput`) in a background thread. Toggles recording on F5, Esc/Enter stops recording. Transcription runs in a separate thread to keep the UI responsive.
...
- Whisper model files are stored in the pywhispercpp cache directory (not in the repo).
